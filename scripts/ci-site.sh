...
BUILD_OUTPUT="$(yq -er '.pages_build_output_dir' "${OCF_ROOT}/wrangler.toml")"
COMPATIBILITY_DATE="$(yq -er '.compatibility_date' "${OCF_ROOT}/wrangler.toml")"
[[ "$PROJECT_NAME" == "open-cluster-foundation" ]] || die "Cloudflare Pages project name must be open-cluster-foundation"
[[ "$BUILD_OUTPUT" == "./dist" ]] || die "Cloudflare Pages build output must be ./dist"
[[ "$COMPATIBILITY_DATE" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}$ ]] || die "Cloudflare compatibility date must use yyyy-mm-dd"
[[ ! -e "${OCF_ROOT}/.openai/hosting.json" ]] || die "OpenAI Sites configuration must not coexist with Cloudflare Pages"
